...
public class CheckLicense {
  ...
  private static final String PRODUCT_CODE = "PDBBTESTDOX";
  ...
  private static boolean isKeyValid(String key) {
    ...
    try {
      ...
      // expiration checks.
      // This might be the case if the key is at the same time a perpetual fallback license for
      // older IDE versions.
      ...
      final byte[] licenseBytes =
          Base64.getMimeDecoder().decode(licensePartBase64.getBytes(StandardCharsets.UTF_8));
      sig.update(licenseBytes);
      if (!sig.verify(
          Base64.getMimeDecoder().decode(signatureBase64.getBytes(StandardCharsets.UTF_8)))) {
        return false;
      }
      // Optional additional check: the licenseId corresponds to the licenseId encoded in the signed
      // license data
      // The following is a 'least-effort' code. It would be more accurate to parse json and then
      // find there the value of the attribute "licenseId"
      final String licenseData = new String(licenseBytes, StandardCharsets.UTF_8);
      return licenseData.contains("\"licenseId\":\"" + licenseId + "\"");
    } catch (Throwable e) {
      e.printStackTrace(); // For debug purposes only. Normally you will not want to print
      // exception's trace to console
    }
    return false;
  }
  ...
  private static boolean isLicenseServerStampValid(String serverStamp) {
    try {
      ...
      // the last parameter of 'createCertificate()' set to 'true' causes the certificate to be
      // checked for
  ...
  private static X509Certificate createCertificate(
    ...
    final X509Certificate cert =
        (X509Certificate) x509factory.generateCertificate(new ByteArrayInputStream(certBytes));
    ...
    try {
      ...
      if (!checkValidityAtCurrentDate) {
        // deliberately check validity on the start date of cert validity period, so that we do not
        // depend on
